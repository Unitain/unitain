...
-- Create function to handle response updates
CREATE OR REPLACE FUNCTION handle_contact_request_response()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER handle_contact_request_response
  BEFORE UPDATE ON contact_requests
  FOR EACH ROW
  WHEN (NEW.response IS NOT NULL AND OLD.response IS NULL)
  EXECUTE FUNCTION handle_contact_request_response();