...
export function AuthCallback() {
  ...
  const completeSignup = async (user: any, params: URLSearchParams) => {
    try {
      ...
      const { data: tosRecord, error: tosError } = await supabase
        ...
        .insert({
          user_id: user.id,
          tos_version_id: params.get('tos_version') || 'unknown',
          ip_address: params.get('ip') || 'unknown',
          device_info: params.get('device') || navigator.userAgent.slice(0, 100)
        })
        .select()
        .single();
  ...
  useEffect(() => {
    const handleAuth = async () => {
      try {
        ...
        console.log("🚀 ~ handleAuth ~ emailFromParams:", emailFromParams)

        if (error) throw new Error(error);

        // 1. Handle email confirmation
        if (token && type === 'signup') {
          // First try to verify with just the token (email optional)
          const { data, error: verifyError } = await supabase.auth.verifyOtp({
            token,
            type: 'signup',
          ...
          // Complete registration with TOS params from URL
          await completeSignup(data.user, params);
          return;
        }
