...
export function Download() {
    ...
    try {
      ...
      if (success) {
        ...
        const { error } = await supabase
          ...
          .eq('id', user.id);
        if (error) {
          toast.error(t('guide.updateError'));
        }
      }
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-br from-primary-50 to-primary-100 rounded-xl p-6">
      ...
      <h2 className="text-xl font-semibold text-primary-900 mb-4">{t('guide.title')}</h2>
      <p className="text-sm text-primary-700 mb-6">{t('guide.description')}</p>
      ...
      >
        ...
        <span className="text-primary-700 font-medium group-hover:text-primary-800 transition-colors duration-200">{t('guide.downloadButton')}</span>
